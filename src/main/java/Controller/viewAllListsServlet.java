...
public class viewAllListsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RaffleDetailHelper rdh = new RaffleDetailHelper();
		List<RaffleDetails> abc = rdh.getListName();
		request.setAttribute("allLists", abc);
		if(abc.isEmpty()){
		request.setAttribute("allLists", " ");
		}
		getServletContext().getRequestDispatcher("/raffle-list-by-user.jsp").forward(request, response);
	}
